...
<body class="app-gradient text-slate-950 dark:text-white transition-colors duration-500">
    ...
    <main class="relative z-10 pt-52 pb-16 px-6 text-center max-w-3xl mx-auto">
        ...
        <div class="inline-block glass-effect px-5 py-2 rounded-full mb-6 shadow-sm">
            <span class="text-[11px] sm:text-xs font-black tracking-[0.25em] uppercase" style="color:${city.color}">${city.nameJa} · Japan City Guide</span>
        </div>
        <h1 class="text-[2.5rem] sm:text-5xl md:text-7xl font-[900] tracking-tight mb-4 text-slate-900 dark:text-white">
            ${city.name}
            <span class="block text-xl sm:text-2xl md:text-3xl font-black mt-2 text-slate-400 dark:text-slate-500">${city.nameJa}</span>
        </h1>
        <p class="text-sm sm:text-base text-slate-500 dark:text-slate-400 font-bold leading-relaxed max-w-xl mx-auto mt-6">
            ${city.desc}
        </p>
    </main>

    <section class="relative z-10 px-6 pb-32 max-w-4xl mx-auto">
        <p class="text-center text-xs font-black tracking-[0.25em] uppercase text-slate-400 mb-10">EXPLORE ${city.nameJa}</p>
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <a href="sakura_${city.id}" class="hub-card no-underline rounded-[2.5rem] p-8 bg-white dark:bg-white/5 border-2 border-rose-100 dark:border-white/10 shadow-lg hover:shadow-2xl hover:border-rose-300 dark:hover:border-rose-500/40 block text-center">
                ...
                <p class="text-[11px] font-black text-rose-400 uppercase tracking-widest mb-2">Sakura Guide</p>
                <h2 class="text-xl sm:text-2xl font-black text-slate-800 dark:text-white mb-3">벚꽃 가이드</h2>
                <p class="text-sm text-slate-400 font-bold leading-relaxed mb-6">개화 일정, 하나미 명소,<br/>벚꽃 시즌 완벽 정리</p>
                <span class="inline-flex items-center gap-2 px-5 py-3 rounded-full text-sm font-black text-white" style="background:#e11d48">
            ...
            <a href="matsuri_${city.id}" class="hub-card no-underline rounded-[2.5rem] p-8 bg-white dark:bg-white/5 border-2 border-sky-100 dark:border-white/10 shadow-lg hover:shadow-2xl hover:border-sky-300 dark:hover:border-sky-500/40 block text-center">
                <div class="text-5xl mb-5">🎆</div>
                <p class="text-[11px] font-black text-sky-500 uppercase tracking-widest mb-2">Matsuri Guide</p>
                <h2 class="text-xl sm:text-2xl font-black text-slate-800 dark:text-white mb-3">마쓰리 가이드</h2>
                <p class="text-sm text-slate-400 font-bold leading-relaxed mb-6">${city.matsuriRegion} 지역 마쓰리<br/>일정 & 하이라이트</p>
                <span class="inline-flex items-center gap-2 px-5 py-3 rounded-full text-sm font-black text-white" style="background:#0ea5e9">
                    보러가기 <i class="fa-solid fa-arrow-right-long"></i>
                </span>
            </a>
            ...
            <a href="hanabi_${city.id}" class="hub-card no-underline rounded-[2.5rem] p-8 bg-white dark:bg-white/5 border-2 border-indigo-100 dark:border-white/10 shadow-lg hover:shadow-2xl hover:border-indigo-300 dark:hover:border-indigo-500/40 block text-center">
                <div class="text-5xl mb-5">🎇</div>
                <p class="text-[11px] font-black text-indigo-500 uppercase tracking-widest mb-2">Hanabi Guide</p>
                <h2 class="text-xl sm:text-2xl font-black text-slate-800 dark:text-white mb-3">하나비 가이드</h2>
                <p class="text-sm text-slate-400 font-bold leading-relaxed mb-6">${city.hanabiRegion} 지역 불꽃놀이<br/>대회 & 관람 포인트</p>
                <span class="inline-flex items-center gap-2 px-5 py-3 rounded-full text-sm font-black text-white" style="background:#6366f1">
                    보러가기 <i class="fa-solid fa-arrow-right-long"></i>
                </span>
            </a>
    ...
    <footer class="relative z-10 py-16 px-6 text-center border-t border-slate-200 dark:border-white/10">
        <p class="text-xs font-medium tracking-widest text-slate-400/60 dark:text-slate-500/50 mb-6 uppercase">
